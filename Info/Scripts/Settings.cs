namespace Server
{
	...
	// and designed. These are items like 'Stormbringer' or 'Conan's Lost Sword'. if false, then these items will retain their
	// appearance and color no matter what is done to them. Setting this to true will allow a player to use items like the Magic
	// Scissors or Magic Hammer to change the appearance of the items, but they will always retain their name.
	...
	// player has to find their corpse and potentially collect their belongings.
	...
	// an enemy player to take the dead character's belongings may be desired.
	...
	// Set to the number 0 to disable. The other values are 1, 2, or 3. When greater than 0, you will
	...
	// (assuming an example base cost of 2,000 gold). If you set the second setting to false, then each time you join a guild,
	...
	// If you set this to 60, then a player can only achieve a maximum of 60 in each of the resistance categories
	// (physical, cold, fire, poison, and energy).
	...
	// This is the minimum and maximum gold that a player character starts with. The most gold a character can begin
	// with is 10,000 gold.
	...
	// often treasure will appear. The maximum is 100. Some enemies will generate a bit of treasure 
	// themselves, like gold or reagents, which this does not affect. Chests will also likely
	...
	// there is a minimum of 20.0. Each ability requires 10 additional points above the previous (EG 70, 80, 90, etc).

		public static double S_SpecialWeaponAbilSkill = 70.0;

	// This number can be set from zero to 10, where 10 will give characters faster skill gain.
	...
	// This whole number can be set from 1-100, where it multiplies the skill gain check on training
	...
	// Setting the below value to true will allow one to craft 1, 10, or 100 items at a time. Skills
	...
	// Setting the below value to true will only apply if the S_CraftMany setting is true.
	...
	// If set to zero, this check does not occur normally. Valid values are between 10 and 90.
	...
	// Some higher level creatures will scale these values down, in order for them to remain defeatable.
	...
	// If true then vendors will sell anything they normally stock. Some items have a default rarity % that this setting does not affect.

		public static bool S_SellAll = false;

	// If true then vendors will buy anything they normally stock. Some items have a default rarity % that this setting does not affect.
	...
	// levels. As gameplay occurs, this value can increase for a vendor as you buy things from them, or pay for services they offer.
	...
	// then specify this dual ownership when choosing an item security level. If false, then co-owners have much
	// more limited permissions as the standard game allows.

		public static bool S_HouseOwners = false;

	// When true, characters can use lawn tools (from architects) to add items to the outside of their home like
	// trees, shrubs, fences, lave, water, and other items. Lawn tools require an amount of gold to place items.
	// If this was previously true and characters placed lawn items, and then you set it to false, the lawn items
	// will refund the gold back to the character's bank box and the lawn tools will be removed from the game.

		public static bool S_LawnsAllowed = false;

	// When true, then the player characters can use remodeling tools (from architects) to add items to their
	...
	// This value can be between 0 and 20. This is the number of extra stabled pets players get (beyond
	// the normal amount of '2'), where anything more will rely on their skills in druidism, taming,
	// veterinary, and herding.
